if [ $# -ge 1 ]; then #Comprueba si contiene al menos 1 argumento
    if [ $1 -ge 15 ]&&[ $1 -le 60 ]; then #Comprueba que la edad introducida está entre 15 y 60
        ...
        if [ $(($birthyear%10)) -gt 0 ]; then #Si el año no termmina en 0 la década es el año de nacimiento menos la edad terminado en 0
            decade=$(($birthyear-(($birthyear%10))))
        else    #Si no hay que restar 10 años a la década (ej: 2021 entra en la década de los 2020s, mientras que 2020 entra en los 2010s)
